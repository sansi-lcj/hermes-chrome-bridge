...
- **Agent tools** — toggle **Tools** to let the agent operate your browser via function calling: **perceive** the page (`get_page_elements`, `read_active_page`, `list_tabs`), **read across tabs** (`read_tab`), **manage tabs** (`group_tabs`, `close_tabs`), and **act** on the page (`click_element`, `type_text`, `scroll_page`, `navigate_to`, `open_url`). Each tool step shows in the tool trail. Write/action tools prompt for **Allow/Deny** by default (an **Auto-run** toggle skips prompts).
- **Scheduled tasks** — run a saved prompt on an interval in the background (a morning digest, a periodic check) and get a desktop notification with the result. Managed in Settings.
